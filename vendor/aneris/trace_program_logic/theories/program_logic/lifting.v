...
Lemma wp_lift_step_fupd s E Φ e1 ζ:
  ...
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace AS) K tp1 tp2 σ1,
    ...
    ∀ e2 σ2 efs, ⌜prim_step e1 σ1 e2 σ2 efs⌝ ={∅}=∗ ▷ |={∅,E}=>
      ∃ δ2 ℓ,  ⌜valid_state_evolution
                extr atr (Some ζ) ℓ (tp1 ++ ectx_fill K e2 :: tp2 ++ efs, σ2) δ2⌝ ∗
        state_interp
...
Lemma wp_lift_stuck E Φ e ζ:
  to_val e = None →
  (∀ (extr : execution_trace Λ) (atr : auxiliary_trace AS) K tp1 tp2 σ,
    ⌜valid_exec extr⌝ -∗
...
Lemma wp_lift_pure_step_no_fork
      `{!AllowsPureStep AS Σ} `{!Inhabited (state Λ)} s E E' Φ e1 ζ:
...
Lemma wp_lift_pure_det_step_no_fork
      `{!AllowsPureStep AS Σ} `{!Inhabited (state Λ)} {s E E' Φ} e1 e2 ζ:
  (∀ σ1, if s is NotStuck then reducible e1 σ1 else to_val e1 = None) →
...
Lemma wp_pure_step_fupd
      `{!AllowsPureStep AS Σ} `{!Inhabited (state Λ)} s E E' ζ e1 e2 φ n Φ :
  PureExec φ n e1 e2 →
  φ →
...
Lemma wp_pure_step_later
      `{!AllowsPureStep AS Σ} `{!Inhabited (state Λ)} s E ζ e1 e2 φ n Φ :
  PureExec φ n e1 e2 →
  φ →
