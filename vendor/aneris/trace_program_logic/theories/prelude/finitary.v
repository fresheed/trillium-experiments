...
Section finite_range_gmap.
  ...
  Proof.
    ...
    assert (Hdecomp: ∃ m1 m2, m = m1 ∪ m2 ∧ dom (gset _) m1 = {[a]} ∧ dom (gset _) m2 = list_to_set l).
    ...
    assert (Hanotin: a ∉ dom (gset K) m2).
    ...
    - apply (IHl (dom _ m2)). repeat split.
  ...
  Local Instance proof_irrel_P D N (m: gmap K nat):
    ProofIrrel (dom (gset _) m = D ∧ map_Forall (λ _ v, v <= N) m).
  Proof. apply make_proof_irrel. Qed.

  Lemma bounded_maps_finite (D: gset K) (N : nat):
    Finite { m : gmap K nat | dom (gset _) m = D ∧ map_Forall (λ _ v, v <= N) m}.
  Proof.
  ...
  Lemma enum_gmap_bounded_spec N D m:
    dom (gset K) m = D ∧ map_Forall (λ (_ : K) (v : nat), v ≤ N) m → m ∈ enum_gmap_bounded D N.
  Proof.
    intros **. unfold enum_gmap_bounded. eapply enumerate_gmaps_spec; done.
  Qed.

  Lemma enum_gmap_dom N D:
    Forall (λ m, dom _ m = D) (enum_gmap_bounded D N).
  Proof.
    assert (Hok: forall l D,
      elements D ≡ₚ l -> Forall (λ m, dom (gset _) m = D) (enumerate_gmaps l N)
    ); last first.
  ...
  Definition enum_gmap_bounded' (D: gset K) (N : nat): list { m : (gmap K nat) | dom (gset _) m = D }:=
    Forall_to_sig _ (enum_gmap_dom N D).

  Lemma enum_gmap_bounded'_spec N D m Hdom:
    dom (gset K) m = D ∧ map_Forall (λ (_ : K) (v : nat), v ≤ N) m → m ↾ Hdom ∈ enum_gmap_bounded' D N.
  Proof.
...
Section finite_range_gmap.
  ...
  Local Instance proof_irrel_range_P D (lr : list A) (m: gmap K A):
    ProofIrrel (dom (gset _) m = D ∧ map_Forall (λ _ v, v ∈ lr) m).
  Proof. apply make_proof_irrel. Qed.

  Lemma bounded_range_maps_finite (D: gset K) (lr: list A):
    Finite { m : gmap K A | dom (gset _) m = D ∧ map_Forall (λ _ v, v ∈ lr) m}.
  Proof.
  ...
  Lemma enum_gmap_range_bounded_spec D m (lr : list A):
    dom (gset K) m = D ∧ map_Forall (λ (_ : K) (v : A), v ∈ lr) m → m ∈ enum_gmap_range_bounded D lr.
  Proof.
    intros **. unfold enum_gmap_range_bounded. eapply enumerate_range_gmaps_spec; done.
  Qed.

  Lemma enum_gmap_range_dom lr D:
    Forall (λ m, dom _ m = D) (enum_gmap_range_bounded D lr).
  Proof.
    assert (Hok: forall l D,
      elements D ≡ₚ l -> Forall (λ m, dom (gset _) m = D) (enumerate_range_gmaps l lr)
    ); last first.
    { rewrite Forall_forall. setoid_rewrite Forall_forall in Hok. by apply Hok. }
    induction l.
    { intros ? Hp. eapply list.Permutation_nil_r in Hp. eauto. apply elements_empty_inv in Hp.
  ...
  Definition enum_gmap_range_bounded' (D: gset K) (lr : list A):
    list { m : (gmap K A) | dom (gset _) m = D }
    := Forall_to_sig _ (enum_gmap_range_dom lr D).

  Lemma enum_gmap_range_bounded'_spec lr D m Hdom:
    dom (gset K) m = D ∧ map_Forall (λ (_ : K) (v : A), v ∈ lr) m → m ↾ Hdom ∈ enum_gmap_range_bounded' D lr.
  Proof.
